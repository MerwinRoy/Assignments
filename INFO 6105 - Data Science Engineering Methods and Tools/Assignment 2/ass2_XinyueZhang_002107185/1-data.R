...
arrange(select(filter(df, Grad.Rate >= 80),X,Grad.Rate),desc(Grad.Rate))
...
df1 <-summarise(df1,max=max(Outstate),min=min(Outstate))
df1

df2 <- group_by(college, Private)
df2 <-summarise(df2,max=max(Outstate),min=min(Outstate))
df2
