...
data_view <- arrange(select(filter(df,Grad.Rate>=80),X,Grad.Rate),desc(Grad.Rate))
data_view
...
##DF:
df_group <- group_by(df,Private)
df_summarize <- summarize(df_group,max=max(Outstate),min=min(Outstate))
df_summarize

##College
college_group <- group_by(college,Private)
college_summarize <- summarize(college_group,max=max(Outstate),min=min(Outstate))
college_summarize
